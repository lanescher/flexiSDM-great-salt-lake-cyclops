...
# This script fits an integrated model for the Great Salt Lake Cyclops 
# and compares the model output with the true distribution and parameters.
...
# sidenote: there is a bug in v1.1.2. Remove covForm to avoid an error.
true$covForm <- NULL # remove 
...
# Some models predict huge values. Truncate at 80%ile to help color scale
q80 <- quantile(all.lamb.sf$mean, 0.80)
all.lamb.sf$mean[which(all.lamb.sf$mean > q80)] <- q80
